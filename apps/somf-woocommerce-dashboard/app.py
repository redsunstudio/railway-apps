...
@app.route('/api/debug')
def debug():
    """Debug endpoint - test WooCommerce API with single subscription fetch"""
    import time
    start = time.time()
    try:
        # Just fetch one subscription to test API connectivity
        data, headers = wc_api_request('subscriptions', {'per_page': 1})
        elapsed = time.time() - start
        return jsonify({
            'success': True,
            'api_time': round(elapsed, 2),
            'total_subscriptions': int(headers.get('X-WP-Total', 0)),
            'sample': data[0].get('status') if data else None
        })
    except Exception as e:
        return jsonify({
            'success': False,
...
@app.route('/api/debug/fetch-all')
def debug_fetch_all():
    """Debug endpoint - fetch all subscriptions directly (bypasses cache)"""
    import time
    start = time.time()
    try:
        logger.info("DEBUG: Starting direct fetch_all_subscriptions")
        data = fetch_all_subscriptions()
        elapsed = time.time() - start
        logger.info(f"DEBUG: Direct fetch completed in {elapsed}s, got {len(data)} subs")
        return jsonify({
            'success': True,
        ...
        logger.error(f"DEBUG: Direct fetch failed: {e}")
        return jsonify({
            'success': False,
            'error': str(e),
            'elapsed': round(time.time() - start, 2)
...
def debug_cache_status():
    ...
    with cache_lock:
        ...
        subs_age = None
        if cache['subscriptions']['timestamp']:
            subs_age = (datetime.now() - cache['subscriptions']['timestamp']).total_seconds()

        return jsonify({
            'subscriptions': {
                'cached': subs_cached,
                'count': subs_count,
                'age_seconds': round(subs_age, 1) if subs_age else None
            },
            'orders_keys': list(cache['orders'].keys()),
